\documentclass[11pt,a4j]{ujarticle} % upLaTeX: ujarticle, pLaTeX: jarticle
\西暦
\usepackage{graphicx}
\usepackage{url}
...
\item プレイヤ名: {\verb+p26x42+}（グループ6）shiwasu
...
\item グループ内での作業分担状況の概要: 
      ...
      遠藤（自分）が課題1のテスト・検証やスライド作成、発表、デバック、敵対的オセロAIの作成を担当，
      堀がテスト・検証やデバック、敵対的オセロAIの作成を担当した．
...
自分の有効盤面だけでなく負けに至る盤面もbook化して学習させることで，序盤の事故をさらに減らせると考える．
WLD証明は残り石差25手の時点で行うのではなく、序盤から行なって精度を上げていくことで負ける場合でもできるだけ取られる石数を減らすことができると考える．
...
\textbf{実行例}: 盤面描画を省いた自作ベンチ {\verb+p26x29.Bench+}（先後を半々で入替）で測定した．
\begin{lstlisting}[caption=実験1・実験2の実行例,label=ex:1a]
$ java p26x29.Bench depth
...
\textbf{実験2の考察}: shuffleは\textbf{同評価値の手の選択順}だけを変える．
ルートの更新条件が {\verb+childValue > alpha+}のため，同点の手が複数あると先に試した
...
収まり統計的に有意でなく，shuffle無が高いのは偶然の範囲だと結論した．shuffleは
勝率向上ではなく，手を決定論的にせず相手に読まれにくくする点だと思った
...
$ java p26x29.Bench c
## 課題1c: p26x29 vs RandomPlayer 200局 (先手後手各100)
...
ordering(1f)も直感に反し浅い探索では逆効果だった．逆にbookはnpsを下げつつ総コストを削減するという直感に反する結果だった．したがって今後も，変更ごとに
ベースラインとの多数対局・プロファイルで定量検証する開発サイクルを徹底したい．

\section{参考文献・謝辞}
本演習で配布されたリスト1〜9（パッケージ {\verb+ap26+} / {\verb+p26x29+}）および
総当たり戦フレームワーク {\verb+ap26.league+} を参照した．
課題1の勝率測定は自作の自動対戦ベンチ {\verb+p26x29.Bench+} により，
