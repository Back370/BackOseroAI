...
/**
 * 1 ゲームを進行させるドライバ。本格的なリーグ戦システム
 * (unit2 の Competition26) の簡略版で、unit1 のローカル動作確認用。
 *
 * <h2>動作</h2>
 * <ol>
 *   <li>2 人のプレイヤー (黒・白) と初期盤面を受け取る</li>
 *   <li>{@link #play()} を呼ぶと、終局まで交互に手を進める</li>
 *   <li>各手番で {@link Player#think(Board)} を呼び、合法性と時間を検査</li>
 *   <li>反則 (時間切れ・不正手・例外) があれば即座に試合終了</li>
 * </ol>
 *
 * <h2>使い方 (main)</h2>
 * 既定では {@link MyPlayer} (黒) と {@link RandomPlayer} (白) を対戦させる。
 * 自分の AI の動作確認には、{@link #main(String[])} を編集して両プレイヤーを
 * 差し替える。
 *
 * <h2>本格システム (unit2/Competition26) との違い</h2>
 * <ul>
 *   <li>1 ゲームだけ実行 (リーグ戦・複数盤面は無し)</li>
 *   <li>プロセス分離なし (両プレイヤーが同 JVM 内で動く)</li>
 *   <li>結果集計は最小限</li>
 * </ul>
 */
public class MyGame {
  ...
  /**
   * エントリポイント。MyPlayer (黒) vs RandomPlayer (白) を 1 ゲーム実行。
   ...
   * 終局まで試合を進める。各手番で:
   * <ul>
   *   <li>{@link Player#think(Board)} で次手を取得 (時間計測込み)</li>
   *   <li>{@link #check(Color, Move, Error)} で時間切れ・不正手を検出</li>
   *   <li>反則があれば {@link Board#foul(Color)} で即座に試合終了</li>
   *   <li>正常なら {@link Board#placed(Move)} で盤面を進める</li>
   * </ul>
